...
def LoadConfig(fileName):
        ...
        fileDecoded = json.load(fileConfig);
        fileConfig.close();
        configDecoded = fileDecoded["configs"][0];
        configDecoded["local_port"] = fileDecoded["localPort"];
...
def generate_pac(configs):
    ...
    proxy = "SOCKS5 127.0.0.1:" + str(configs["local_port"]) + "; SOCKS 127.0.0.1:" + str(configs["local_port"]) + "; DIRECT";
...
"""%(proxy, configs["server"])
    rfile.write(strLines)
    rfile.close()
    print ("Rules: %d items.\n"
           "Usage: Use the newly created %s as your shadowsocks-gui pac file." %(intLines, pacfile))
